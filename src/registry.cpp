...
void Registry::viewes(account_name provider, uint64_t from, uint64_t to) {
    ...
    while (endpointsIterator != endpoints.end() && counter <= (to - from)) {
        ...
        std::string functions = Registry::vector_to_string(item.functions);
        print_f("endpoint #%: provider = %, specifier = %, functions = %.\n", counter, name{item.provider}, item.specifier, functions);
...
void Registry::endpbyhash(account_name provider, std::string specifier) {
     ...
     std::string functions = Registry::vector_to_string(item.functions);
     print_f("endpoint: provider = %, specifier = %, functions = %.\n", name{item.provider}, item.specifier, functions);
}
